      ...
      function renderPetModal(pet) {
        ...
        getAvailableForms(pet).then(async forms => {
          ...
          const hatchInfoSection = document.querySelector('.hatch-info-section')
          if (hatchText === '未知' && heightText === '未知' && weightText === '未知') {
            hatchInfoSection.style.display = 'none'
          } else {
            hatchInfoSection.style.display = 'block'
          }
          document.getElementById('info-trait').textContent = traitName
          document.getElementById('info-trait-desc').textContent = traitDesc
          document.getElementById('moves-count-new').textContent = `共 ${allMoves.length} 个`
          ...
          const statsHtml = stats.map(stat => `
            <div class="stat-row-new">
              <span class="stat-label-new">${stat.label}</span>
              <div class="stat-bar-container-new">
                <div class="stat-bar-new" style="width: ${(stat.value / maxStat) * 100}%; background: ${stat.color}"></div>
              </div>
              <span class="stat-value-new" id="${stat.id}">${stat.value}</span>
            </div>
          `).join('')
          ...
          function renderMovesPage() {
            ...
            const grid = document.getElementById('moves-grid-new')
            grid.innerHTML = pageItems.map(move => renderMoveItem(move)).join('')
            
            const pageInfo = document.getElementById('moves-page-info-new')
            const prevBtn = document.getElementById('moves-prev-new')
            const nextBtn = document.getElementById('moves-next-new')
          ...
          document.getElementById('moves-prev-new').addEventListener('click', () => {
            if (paginationState.currentPage > 1) {
              paginationState.currentPage--
              renderMovesPage()
            }
          })
          document.getElementById('moves-next-new').addEventListener('click', () => {
          ...
          if (pet.is_leader_form) {
            ...
            const renderEvoNode = (evoPet, showArrow = false, arrowClass = '') => {
              ...
              return `
                <div class="evo-node ${isCurrent ? 'active' : ''}">
